...
cnpj = re.search(cnpj_pattern, texto_documento)

print(f"cnpj: {cnpj.group(0)}")

palavra_chave = ["dados", "identificação"]

n_da_empresa = ["razão social", "nome/razão social", "nome do prestador", "nome social"]

prestador = ['prestador', 'emitente']

tomador = ["tomador", "destinatário", "cliente", "contratante"]
...
for linha in texto_separado:
    #Tratamento da linha de texto
    linha_tratada = linha.lower().strip()
    for palavra in n_da_empresa:
        if palavra in linha_tratada:
            linha_tratada = linha_tratada.replace(palavra, "")
            if ":" in linha_tratada:
                linha_tratada = linha_tratada.replace(":", "")
            empresas += [linha_tratada.strip()]
print(empresas)
